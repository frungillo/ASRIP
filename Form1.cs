...
namespace ASRIP
{
    public partial class Form1 : Form
    {
        ...
        {
            string sql = @"VARIAZIONI_CODICE like'*{0}*'  or NOMINATIVO like '*{0}*' or VARIAZIONI_NOTE like '*{0}*'";
        ...
        {
            string sql = $@"SELECT 
                A.VARIAZIONI_CODICE_BDROP||'-'||A.VARIAZIONI_CODICE_ANM as VARIAZIONI_CODICE, A.VARIAZIONI_DA_DATA, 
                A.VARIAZIONI_A_DATA, A.VARIAZIONI_MATRICOLA ||' - '|| (select cognome||' '|| nome from paghenet.arcdipan where coddip =A.VARIAZIONI_MATRICOLA) as NOMINATIVO, 
                A.VARIAZIONI_DEP_INTERESSATO, A.VARIAZIONI_LINEA, A.VARIAZIONI_TRENO, 
                A.VARIAZIONI_MONTO, A.VARIAZIONI_SCAMBIA_CON, A.VARIAZIONI_FLAG_CONSENSO, 
                A.VARIAZIONI_UTENTE, LOWER(A.VARIAZIONI_NOTE) as VARIAZIONI_NOTE, 
                 A.VARIAZIONI_NUM_PROTOCOLLO, 
                A.DATA_COM_EVENTO
            FROM ANMIS1.ANM_VROS_D_VARIAZIONI A where VARIAZIONI_CODICE_BDROP=6 and to_date('{txtData.Value.ToShortDateString()}','dd/mm/yyyy')
            ...
            variazioni_matricola in (select Matricolaautista 
                                        ...
                                        where residenza in ('CAosta','CAosta2','CarloIII','Garittone','Posillipo','SGiovann','StPolare','VPuglie','VPuglie2')
